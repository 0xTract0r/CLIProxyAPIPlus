...
func TestManagerExecute_AntigravityInvalidGrantFallsBackAndSuspendsAuth(t *testing.T) {
	...
	badAuth := &Auth{ID: "aa-bad-auth", Provider: "antigravity", ProxyURL: "http://test-proxy:8080"}
	goodAuth := &Auth{ID: "bb-good-auth", Provider: "antigravity", ProxyURL: "http://test-proxy:8080"}
...
func TestManager_RequestScopedErrorStopsCredentialFallbackWithoutSuspendingAuth(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			model := "gpt-5.5"
			badAuth := &Auth{ID: "aa-bad-auth", Provider: provider, ProxyURL: "http://test-proxy:8080"}
			goodAuth := &Auth{ID: "bb-good-auth", Provider: provider, ProxyURL: "http://test-proxy:8080"}
...
func TestManager_ExecuteCount_GenericRouteNotFoundDoesNotSuspendModel(t *testing.T) {
	...
	auth := &Auth{ID: "count-route-not-found-auth", Provider: "claude", ProxyURL: "http://test-proxy:8080"}
	reg := registry.GetGlobalRegistry()
...
func TestManager_ExecuteCount_ExplicitModelNotFoundSuspendsModel(t *testing.T) {
	...
	auth := &Auth{ID: "count-model-not-found-auth", Provider: "claude", ProxyURL: "http://test-proxy:8080"}
	reg := registry.GetGlobalRegistry()
	reg.RegisterClient(auth.ID, auth.Provider, []*registry.ModelInfo{{ID: model}})
	t.Cleanup(func() { reg.UnregisterClient(auth.ID) })
...
func TestManager_Execute_GenericRouteNotFoundStillSuspendsModel(t *testing.T) {
	...
	auth := &Auth{ID: "messages-route-not-found-auth", Provider: "claude", ProxyURL: "http://test-proxy:8080"}
	reg := registry.GetGlobalRegistry()
	reg.RegisterClient(auth.ID, auth.Provider, []*registry.ModelInfo{{ID: model}})
	t.Cleanup(func() { reg.UnregisterClient(auth.ID) })
	if _, errRegister := m.Register(context.Background(), auth); errRegister != nil {
...
func TestManager_RecordResult_AvailabilityNeutralSkipsSchedulerUpdate(t *testing.T) {
	m := NewManager(nil, nil, nil)
	auth := &Auth{ID: "availability-neutral-auth", Provider: "claude", ProxyURL: "http://test-proxy:8080"}
	if _, errRegister := m.Register(context.Background(), auth); errRegister != nil {
		t.Fatalf("register auth: %v", errRegister)
	}
