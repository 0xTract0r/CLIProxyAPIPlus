...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (cliproxyexecutor.Response, error) {
	...
	for {
		...
		for _, upstreamModel := range models {
			...
			// Fork codex proactive quota cooldown: if a successful codex response's
			// quota headers already report the account exhausted, mark it so the auth
			// cools down and rotates off before it starts returning 429s.
			if retryAfter := quotaRetryAfterFromHeadersNow(provider, resp.Headers); retryAfter != nil {
...
func (m *Manager) HttpRequest(ctx context.Context, auth *Auth, req *http.Request) (*http.Response, error) {
	...
	// Inject the per-auth RoundTripper so the executor's HTTP call routes through the
	// account's proxy/custom transport instead of the shared default (fork anti-corr:
	// out-of-band management/probe traffic must egress on the same per-account path).
	if rt := m.roundTripperFor(auth); rt != nil {
		execCtx = context.WithValue(execCtx, roundTripperContextKey{}, rt)
		execCtx = context.WithValue(execCtx, "cliproxy.roundtripper", rt)
	}
