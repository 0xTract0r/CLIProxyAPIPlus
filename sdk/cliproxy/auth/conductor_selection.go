...
func (m *Manager) availableAuthsForRouteModel(auths []*Auth, provider, routeModel string, now time.Time) ([]*Auth, error) {
	...
	for _, candidate := range auths {
		...
		// serve the requested route (or its selection) model before they can be picked.
...
func (m *Manager) authSupportsRouteModel(registryRef *registry.ModelRegistry, auth *Auth, routeModel string) bool {
	...
	// Fork plan gate: an account that is not allowed to serve the route model does
	// not "support" it regardless of what the registry snapshot lists.
...
func (m *Manager) pickNextLegacy(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
	...
	for _, candidate := range m.auths {
		...
		if !m.authAllowsClaudeContextRequest(candidate, model, opts) {
			continue
		}
...
func (m *Manager) pickNext(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
	...
	disallowFreeAuth := disallowFreeAuthFromMetadata(opts.Metadata)
	tried = m.excludeClaudeContextAuths(model, opts, tried)
	for {
