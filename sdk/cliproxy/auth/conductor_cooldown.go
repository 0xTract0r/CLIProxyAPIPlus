...
// 429 responses (e.g. model capacity, TPM bursts, context_length) so other
// auths can be tried while this one recovers within a short window, WITHOUT
// flipping plan-level Quota.Exceeded state. Fork-only anti-burn behavior:
// upstream v7.2.101 cools every 429 as escalating plan quota via
// quotaCooldownAfterFailure. Kept semantically distinct from
// transientErrorCooldownSeconds (which governs 408/5xx transport errors).
// Pinned by scheduler_test.go's transient-429 guard tests.
...
func (m *Manager) setConfigSnapshotLocked(cfg *internalconfig.Config) bool {
	...
	// reload. setGlobalProxyConfigured only takes the scheduler's own lock, so it
	// is safe to call here while holding configCooldownMu (m.mu is not held at
	// this point) -- this mirrors the fork's SetConfig wiring.
	if m.scheduler != nil {
...
func (m *Manager) MarkResult(ctx context.Context, result Result) {
	...
	if auth, ok := m.auths[result.AuthID]; ok && auth != nil {
		...
		if result.Success {
			if result.Model != "" {
				...
				if result.QuotaExceeded && result.RetryAfter != nil && *result.RetryAfter > 0 {
					// Fork codex proactive quota cooldown: a "successful" response whose
					// quota headers already report exhaustion cools the model down now so
					// the scheduler rotates off this auth before it starts 429ing (see
					// quotaRetryAfterFromHeaders wired into the execute/stream paths).
					// Unavailable/Status/StatusMessage must be set here: isAuthBlockedForModel
					// only treats a model state as cooled when state.Unavailable is true, so
					// omitting it leaves the exhausted auth schedulable and pickSingle keeps
					// selecting it over healthy accounts.
			...
			if result.Model != "" {
				if isLongContextExtraUsageRequiredResultError(result.Error) {
					// Fork Claude 1M capability: the long-context extra-usage 429 is a
					// per-request signal (the credential itself is healthy and the model
					// stays available), so keep the model active with a clear status
					// message instead of cooling it down as a quota/transient failure.
					// This also keeps the auth off the terminal-auth quarantine path
					// (see isTerminalAuthQuarantineResultError, which excludes it).
					state := ensureModelState(auth, result.Model)
					state.Unavailable = false
					...
					if isModelSupportResultError(result.Error) {
						...
						case 429:
							// MERGE-REVIEW: reconcile the fork's plan-quota vs transient 429
							// split with upstream's escalating quota backoff. A 429 counts as
							// a plan-level quota exhaustion when the upstream supplies a
							// RetryAfter hint OR the error explicitly signals a usage/plan
							// quota (see resultIndicatesPlanQuota); those flip Quota.Exceeded
							// and either honor the RetryAfter or escalate via
							// quotaCooldownAfterFailure (upstream behavior). Every other 429
							// (model capacity / TPM burst / context_length, e.g. a bare
							// "rate_limit") is a brief transient cooldown that must NOT flip
							// plan-level quota state, so the auth recovers fast without being
							// marked quota-exhausted (fork anti-burn). Pinned by
							// scheduler_test.go transient-429 tests +
							// cooldown_backoff_test/error_events_test plan-quota tests.
							hasRetryAfter := result.RetryAfter != nil && *result.RetryAfter > 0
							isPlanQuota := hasRetryAfter || resultIndicatesPlanQuota(result.Error)
							var next time.Time
							backoffLevel := state.Quota.BackoffLevel
							if !disableCooling {
								if isPlanQuota {
		...
		// auth-level branches), so it is always the final word for this call and
		// can never be silently overwritten by the generic "auth.Status =
		// StatusError" writes those branches perform for every kind of failure.
		// See evaluateAutoQuarantineLocked for the recovery guarantee (a real
...
func isRequestInvalidError(err error) bool {
	...
	case http.StatusBadRequest:
		msg := err.Error()
		// image_generation_user_error is a request-level error (e.g. codex-cli's
		// built-in image_generation tool referencing an upstream gpt-image-2 model
		// that does not exist). It is triggered by the request body itself, so
		// switching accounts or upstream models cannot fix it; classify it as a
		// non-retryable request-level error to avoid triggering a credential
		// rotation loop.
