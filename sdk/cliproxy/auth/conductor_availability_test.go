...
func TestManager_AvailableProvidersAndHasProviderAuth_ExcludeDisabled(t *testing.T) {
	...
	if _, err := manager.Register(ctx, &Auth{ID: "active", Provider: "claude", ProxyURL: "http://test-proxy:8080", Status: StatusActive}); err != nil {
		t.Fatalf("register active auth: %v", err)
	}
	// Provider gemini only has an auth with the Disabled flag set.
	if _, err := manager.Register(ctx, &Auth{ID: "flag-disabled", Provider: "gemini", ProxyURL: "http://test-proxy:8080", Disabled: true}); err != nil {
		t.Fatalf("register flag-disabled auth: %v", err)
	}
	// Provider codex only has an auth whose Status is StatusDisabled.
	if _, err := manager.Register(ctx, &Auth{ID: "status-disabled", Provider: "codex", ProxyURL: "http://test-proxy:8080", Status: StatusDisabled}); err != nil {
