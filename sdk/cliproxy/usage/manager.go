...
type Detail struct {
	...
	// MERGE-REVIEW: fork uses CacheWriteTokens, upstream renamed it to
	// CacheCreationTokens. Both names are referenced across the tree (fork
	// pricing/usage helpers vs upstream redisqueue/usage helpers), so both
	// fields are kept to preserve compilation. They map to the same upstream
	// cache_creation_input_tokens value and should be deduplicated later.
