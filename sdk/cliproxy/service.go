...
func (s *Service) applyCoreAuthRemoval(ctx context.Context, id string) {
	...
	// MERGE-REVIEW: fork previously soft-disabled the auth here (kept the entry
	// with Disabled=true and re-bound codex executors). Upstream v7 switched to a
	// hard removal (Remove + UnregisterClient), which also satisfies the fork
	// "delete -> re-add does not inherit stale runtime state" regression. Adopting
	// upstream removal; revisit if any fork path depends on a retained disabled
	// auth entry after removal.
...
func (s *Service) Run(ctx context.Context) error {
	...
	if !homeEnabled {
		...
		// Connect the Kiro background refresh manager callback to the watcher so
		// that a successful background token refresh immediately updates the
		// in-memory Auth object, closing the timing gap between refresh and reload.
