...
func baselineExecutorAuths() []*coreauth.Auth {
	providers := []string{
		...
		// fork: gemini-cli is a fork-added provider whose executor carries the
		// anti-correlation outbound behavior; it must be in the home/baseline set
		// so home-dispatched gemini-cli auths can execute without local creds.
		"gemini-cli",
		"vertex",
		"aistudio",
		"antigravity",
...
// registerHomeExecutors registers the baseline executor set so home-dispatched
// auth entries can execute without local auth-dir credentials. The fork retains
// this entry point (upstream folded the equivalent into registerAvailableExecutors
// + baselineExecutorAuths); it delegates to the unified baseline path so the
// gemini-cli home-registration guard (F6) keeps covering this binding.
func (s *Service) registerHomeExecutors() {
	if s == nil || s.coreManager == nil {
		return
	}
	s.registerAvailableExecutors(context.Background(), executorRegistrationOptions{includeBaseline: true})
}
...
func (s *Service) registerExecutorForAuth(a *coreauth.Auth, forceReplace bool) {
	...
	if strings.EqualFold(strings.TrimSpace(a.Provider), "codex") {
		...
		// fork(anticorr): inject the core manager so the Codex auto executor can
		// count cyber_policy hits and write quota/webhook state back (needed when
		// GitLab Duo / others fall back to the Codex OpenAI gateway).
		s.coreManager.RegisterExecutor(executor.NewCodexAutoExecutorWithManager(cfg, s.coreManager))
		return
	}
	...
	case "claude":
		// fork(anticorr): manager-aware Claude executor for quota/cyber_policy writeback.
		s.coreManager.RegisterExecutor(executor.NewClaudeExecutorWithManager(cfg, s.coreManager))
	case "kimi":
		s.coreManager.RegisterExecutor(executor.NewKimiExecutor(cfg))
	case "gemini-cli":
		// fork: gemini-cli executor carries the fork's anti-correlation outbound
		// behavior for gemini-cli auths (guarded by F6 binding test).
	...
	case "gitlab":
		// gitlab Duo falls back to the Codex OpenAI gateway and hits cyber_policy,
		// so the manager must be injected for counting and webhook writeback.
		s.coreManager.RegisterExecutor(executor.NewGitLabExecutorWithManager(cfg, s.coreManager))
	case "xai":
		if !forceReplace {
