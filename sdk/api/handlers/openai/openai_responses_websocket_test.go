...
func TestResponsesWebsocketCodexWebsocketPassthroughPassesCompactedRequestWithoutTranscriptMerge(t *testing.T) {
	...
	auth := &coreauth.Auth{
		...
		// "direct" keeps the account schedulable under the fork anti-correlation
		// proxy_url guard (scheduler.go authMissingProxyURL).
		ProxyURL: "direct",
	}
...
func TestResponsesWebsocketFailedProviderRoutePreservesNativeWebsocketPin(t *testing.T) {
	...
	manager.RegisterExecutor(codexExecutor)
	manager.RegisterExecutor(claudeExecutor)
	codexAuth := &coreauth.Auth{ProxyURL: "direct", ID: "auth-codex-provider-route-failure", Provider: "codex", Status: coreauth.StatusActive, Attributes: map[string]string{"websockets": "true"}}
	claudeAuth := &coreauth.Auth{ProxyURL: "direct", ID: "auth-claude-provider-route-failure", Provider: "claude", Status: coreauth.StatusActive}
	for _, auth := range []*coreauth.Auth{codexAuth, claudeAuth} {
		if _, err := manager.Register(context.Background(), auth); err != nil {
			t.Fatalf("Register auth %s: %v", auth.ID, err)
		}
...
func TestResponsesWebsocketDeltaRouteToBuiltInProviderRequiresFullReplay(t *testing.T) {
	...
	manager.RegisterExecutor(codexExecutor)
	manager.RegisterExecutor(claudeExecutor)
	codexAuth := &coreauth.Auth{ProxyURL: "direct", ID: "auth-codex-provider-route-delta", Provider: "codex", Status: coreauth.StatusActive, Attributes: map[string]string{"websockets": "true"}}
	claudeAuth := &coreauth.Auth{ProxyURL: "direct", ID: "auth-claude-provider-route-delta", Provider: "claude", Status: coreauth.StatusActive}
	for _, auth := range []*coreauth.Auth{codexAuth, claudeAuth} {
		if _, err := manager.Register(context.Background(), auth); err != nil {
			t.Fatalf("Register auth %s: %v", auth.ID, err)
		}
...
func TestResponsesWebsocketPrewarmHandledLocallyForSSEUpstream(t *testing.T) {
	...
	manager := coreauth.NewManager(nil, nil, nil)
	manager.RegisterExecutor(executor)
	auth := &coreauth.Auth{ProxyURL: "direct", ID: "auth-sse", Provider: executor.Identifier(), Status: coreauth.StatusActive}
	if _, err := manager.Register(context.Background(), auth); err != nil {
		t.Fatalf("Register auth: %v", err)
	}
	registry.GetGlobalRegistry().RegisterClient(auth.ID, auth.Provider, []*registry.ModelInfo{{ID: "test-model"}})
