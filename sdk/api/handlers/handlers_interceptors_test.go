...
func newInterceptorHandler(t *testing.T, model string, executor *interceptorCaptureExecutor, cfg *sdkconfig.SDKConfig) *BaseAPIHandler {
	...
	auth := &coreauth.Auth{
		// ProxyURL "direct" is the fork anti-correlation sentinel: auths with an empty
		// proxy_url are dropped from scheduling (scheduler.go authMissingProxyURL) to avoid
		// IP-exposing direct egress. Upstream interceptor tests predate that guard, so set
		// the explicit direct sentinel to keep the test account schedulable.
		ProxyURL: "direct",
