...
func newModelExecutionHandler(t *testing.T, model string, executor *modelExecutionCaptureExecutor, cfg *sdkconfig.SDKConfig) *BaseAPIHandler {
	...
	auth := &coreauth.Auth{
		// "direct" is the fork anti-correlation sentinel: an empty proxy_url drops the
		// auth from scheduling (scheduler.go authMissingProxyURL). Keep the test account
		// schedulable with an explicit direct egress sentinel.
		ProxyURL: "direct",
