...
func isOpenAIFamily(provider string) bool {
	switch provider {
	// github-copilot serves OpenAI models over the OpenAI/Codex wire protocol, so
	// it belongs to the OpenAI family. Classifying it here keeps the upstream
	// modelFamilyMismatch clamp scoped to genuine cross-protocol reuse (e.g. Kimi
	// on /v1/messages) while preserving strict per-model level validation for
	// github-copilot models (an unsupported level such as gpt-5 "xhigh" must error,
	// not silently clamp to "high").
