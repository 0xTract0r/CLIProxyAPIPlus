...
func (m configTabModel) parseConfig(cfg map[string]any) []configField {
	...
	fields = append(fields, configField{"Logs Compress After Days", "logs-compress-after-days", "int", fmt.Sprintf("%.0f", getFloat(cfg, "logs-compress-after-days")), nil})
	fields = append(fields, configField{"Logs Delete After Days", "logs-delete-after-days", "int", fmt.Sprintf("%.0f", getFloat(cfg, "logs-delete-after-days")), nil})
...
func fieldSection(apiPath string) string {
	...
	case "logging-to-file", "logs-max-total-size-mb", "logs-compress-after-days", "logs-delete-after-days", "error-logs-max-files", "usage-statistics-enabled", "request-log":
