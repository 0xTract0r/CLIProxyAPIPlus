...
func isKnownDefaultValue(path []string, node *yaml.Node) bool {
	...
	// fork(anticorr): quota-snapshot-refresh.enabled / .startup-catch-up default to
	// true, so an explicit `false` must be checked BEFORE the generic zero pruning
	// below — otherwise the false flags are dropped on save (regression fixed by
	// TestSaveConfigPreserveCommentsKeepsQuotaSnapshotRefreshFalseFlags). Only the
	// redundant `true` is a prunable known default here.
	...
	// Generic zero-value pruning for everything not special-cased above.
...
// removeLegacyAmpKeys strips the deprecated flat Amp keys that were superseded by
// the nested `ampcode` block. It intentionally preserves the live `ampcode` node so
// its comments/ordering survive the comment-preserving save (fork behavior).
