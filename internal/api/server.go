...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		mgmt.DELETE("/oauth-session", s.mgmt.CancelOAuthSession)
	}
}
...
func applySignatureCacheConfig(oldCfg, cfg *config.Config) {
	newEnabled := configuredSignatureCacheEnabled(cfg)
	newStrict := configuredSignatureBypassStrict(cfg)
	if oldCfg == nil {
		cache.SetSignatureCacheEnabled(newEnabled)
		cache.SetSignatureBypassStrictMode(newStrict)
		return
	}

	if configuredSignatureCacheEnabled(oldCfg) != newEnabled {
		cache.SetSignatureCacheEnabled(newEnabled)
	}
