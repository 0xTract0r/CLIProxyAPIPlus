...
func TestManagementUsageRequiresManagementAuthAndPopsArray(t *testing.T) {
	...
	// Fork behavior: `/v0/management/usage` is the fork's usage-statistics
	// endpoint (consumed by apps/web), not the upstream legacy usage-queue
	// alias. Upstream renamed its old queue route from `/usage` to
	// `/usage-queue` and treats the bare `/usage` path as removed (404), but in
	// this fork the path is repurposed for usage statistics and must stay 200.
	...
	if legacyRR.Code != http.StatusOK {
		t.Fatalf("fork usage-statistics status = %d, want %d body=%s", legacyRR.Code, http.StatusOK, legacyRR.Body.String())
	}
...
// TestAmpProviderModelRoutes asserts the fork-only Amp module registers its
// per-provider model listing routes (/api/provider/<provider>/models). It guards
// against the Amp module being silently dropped during upstream restructures.
