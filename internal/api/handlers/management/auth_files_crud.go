...
func (h *Handler) buildAuthFromFileData(path string, data []byte) (*coreauth.Auth, error) {
	...
	// Fork anti-corr: restore per-account runtime identity fields (proxy_url and
	// the synthetic Claude device_id that masks the real machine device_id) from
	// metadata. ApplyRuntimeFieldsFromMetadata only fills empty fields, so it is
	// safe to run after the synthesizer path above.
