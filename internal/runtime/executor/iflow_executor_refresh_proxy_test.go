...
// TestIFlowExecutor_Refresh_OAuthBased_UsesAccountProxyURL verifies the
// OAuth-token refresh path routes through auth.ProxyURL rather than falling
// back to the global cfg.ProxyURL.
//
// Regression coverage for the bug where the per-account proxy was silently
// ignored at refresh time.
func TestIFlowExecutor_Refresh_OAuthBased_UsesAccountProxyURL(t *testing.T) {
	...
	if got := atomic.LoadInt32(&accountProxyHits); got == 0 {
		t.Fatalf("expected IFlowExecutor.Refresh (OAuth) to route through account proxy %s, but proxy received no CONNECT", accountProxy.URL)
	}
}
