...
func TestKimiExecutorClaudeStreamForwardsAnthropicBetaAndLogsUpstream(t *testing.T) {
	...
	// When a gin context is present, applyClaudeHeaders resolves the client's
	// anthropic-beta set from the inbound gin request (ginHeadersFromContext prefers
	// the real request over the fallback inbound-headers context key). In production
	// the inbound API request carries Anthropic-Beta, so mirror that here; the
	// executor unions these client betas with the maintained fork floor.
	ginCtx.Request.Header.Set("Anthropic-Beta", "client-beta-one, client-beta-two")
