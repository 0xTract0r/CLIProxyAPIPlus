...
func TestAntigravityExecutorExecuteStreamTranslatesInteractionsRequest(t *testing.T) {
	...
	auth := &cliproxyauth.Auth{
		...
		// ProxyURL "direct" satisfies the fork anti-correlation egress guard
		// (helps.newProxyAwareHTTPClient blocks account egress when proxy_url is
		// empty to prevent IP exposure); "direct" is the explicit-operator sentinel
		// allowing a real dial to the loopback httptest server.
		ProxyURL: "direct",
