...
func TestAntigravityReasoningReplayClearsOnInvalidSignature400(t *testing.T) {
	...
	auth := &cliproxyauth.Auth{
		...
		// ProxyURL "direct" satisfies the fork anti-correlation egress guard so the
		// request actually reaches the loopback httptest server and returns the 400
		// that drives the invalid-signature replay-cache clear under test.
		ProxyURL: "direct",
