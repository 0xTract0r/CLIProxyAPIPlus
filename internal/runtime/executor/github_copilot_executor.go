...
func (e *GitHubCopilotExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	go func() {
		...
		for scanner.Scan() {
			...
			var chunks [][]byte
			if useResponses && from.String() == "claude" {
				translated := translateGitHubCopilotResponsesStreamToClaude(bytes.Clone(line), &param)
				chunks = make([][]byte, 0, len(translated))
...
func (e *GitHubCopilotExecutor) applyHeaders(r *http.Request, apiToken string, body []byte, auth *cliproxyauth.Auth) {
	...
	// Codex/Claude/Kimi/Kilo executor pattern.
	if auth != nil {
