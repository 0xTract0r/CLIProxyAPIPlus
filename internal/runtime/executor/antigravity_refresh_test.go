...
func TestAntigravityRefresh_DeduplicatesConcurrentRefresh(t *testing.T) {
	...
	// The fork anti-correlation egress guard (helps.NewProxyAwareHTTPClient) blocks
	// account HTTP clients whose proxy_url is empty to prevent IP exposure, which
	// would keep the refresh from ever reaching the loopback httptest server. Inject
	// the test transport as the deliberate, caller-controlled egress path
	// (cliproxy.roundtripper) — the guard explicitly treats that as an allowed
	// non-accidental egress decision — so the refresh dials the test /token endpoint.
	testRoundTripper := &http.Transport{
		DialContext: func(ctx context.Context, network, addr string) (net.Conn, error) {
	...
	refreshCtx := context.WithValue(context.Background(), "cliproxy.roundtripper", http.RoundTripper(testRoundTripper))
