...
func parseOpenAIStyleUsageNode(usageNode gjson.Result) usage.Detail {
	...
	if cached.Exists() {
		...
		// MERGE-REVIEW: fork billing increment — keep CacheReadTokens populated
		// alongside CachedTokens. Required for fork usage/pricing.
...
// parseOpenAIResponsesUsageDetail and the ParseOpenAIResponses* helpers below are
// fork-unique (upstream has no OpenAI Responses usage parsing); they are still
// referenced via the helps exported surface and must be preserved.
...
// hasGeminiFamilyUsageTokenFields reports whether a usageMetadata node carries any
// Gemini-family token counters. Fork-unique helper used by the Gemini CLI usage
// parsing below (upstream dropped the CLI variants during the split).
...
// ParseGeminiCLIUsage parses usage from a Gemini CLI response body. Fork-unique:
// the Gemini CLI wraps usageMetadata under a top-level "response" object in
// addition to the bare forms handled by ParseGeminiUsage.
...
// ParseGeminiCLIStreamUsage parses usage from a Gemini CLI stream line. Fork-unique
// counterpart to ParseGeminiCLIUsage for streaming responses.
