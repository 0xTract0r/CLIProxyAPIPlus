...
// fork(anticorr): keep "account_remark" (populated by codex_executor.go) right after
// "provider" so per-account remarks stay visible in human logs, while adopting the
// upstream plugin/credential/media log fields.
var logFieldOrder = []string{
	"provider", "account_remark", "model",
