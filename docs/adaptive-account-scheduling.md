...
Fresh-child recognition accepts a single text user task, optionally followed by exactly one observed CLI auxiliary `system` message with one text block and optional supported ephemeral cache control. The auxiliary text can be the exact `Today's date is YYYY-MM-DD.` notice, or the fixed CLI deferred-tools catalog, skills catalog, or both in that order, ending with the same valid date. Tool catalogs require the complete framework introduction and nonempty valid tool-name lines; skill catalogs require the exact framework heading and named, nonempty entries, including namespaced names and multiline descriptions. Other paragraphs, unknown headings, reordered or duplicate sections, empty catalogs, invalid dates, trailing content, media, extra history or fork evidence do not qualify.

Catalog descriptions are opaque text data, including observed unindented continuation lines within an entry. Structural recognition cannot prove their provenance or semantic trustworthiness; it is not a security filter. Parent and child catalogs can differ. The auxiliary message is excluded only from the single-task identity test: original request bytes, headers, tools, existing hash inputs, message counts, input cost, cache TTL and migration eligibility retain their existing handling. Existing child bindings are not reclassified or given another reserve draw when this grammar becomes supported.

Parent-history inspection distinguishes content blocks from metadata and tool arguments. It recognizes `tool_use.caller = {"type":"direct"}` and `tool_reference` blocks with a string `tool_name` only inside `tool_result.content`; a business `type` field inside `tool_use.input` is ordinary JSON data. Unknown callers, unknown content, media and identity conflicts remain conservative. Recognition applies only to unbound children: existing parent-affine children are not reclassified, resumed children keep their binding, and a child's selection never overwrites its parent's binding.

Recognized text `system` notices anywhere in the parent's message history remain ordinary history, including string content and text-block arrays. The original request and existing hash inputs remain unchanged; message counts, input cost and cache TTL retain their existing accounting. Only the first message, top-level system and tools have hashes; middle or trailing notices are not individually hashed. This does not relax the fresh-child rule: a child's arbitrary system notice or inherited history still fails the single-task test; unknown structures and media remain conservative.

Identity recognition and migration-cost eligibility are separate. Recognizing these additional tool shapes does **not** expand the previous conservative migration-input whitelist; such a parent can establish a fresh child's identity while still being ineligible for proactive migration itself.
