...
Fresh-child recognition accepts a single text user task, optionally followed by exactly one observed CLI date message: a `system` message containing one text block with `Today's date is YYYY-MM-DD.` and optional supported ephemeral cache control. Other system text, extra messages or fork evidence do not qualify. The date is excluded only from the single-task identity test; it still contributes to message count, request cost and cache-TTL evidence.
