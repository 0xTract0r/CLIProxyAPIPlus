...
独立子代理首轮允许一条纯文本user任务，以及可选的一条实测CLI辅助`system`消息：content仅含一个text块，可带已支持的ephemeral缓存标记。正文支持严格的`Today's date is YYYY-MM-DD.`日期通知，或CLI固定格式的deferred-tools目录、skills目录、按工具后技能顺序组合的双目录，末尾都必须有同格式的合法日期。工具目录要求完整固定前导及非空合法工具名逐行排列；技能目录要求固定标题及命名、非空描述的条目，支持命名空间名称和多行描述。其他段落、未知标题、章节乱序或重复、空目录、错误日期、尾随内容、媒体、额外历史及fork证据都不满足独立首轮条件。

目录描述作为不执行的文本数据保留，支持条目内实测的无缩进续行。结构识别不能证明其来源或语义可信，不是安全过滤器；父子代理的目录也不要求相同。辅助消息只在“是否单任务首轮”的身份判断中排除：原始请求字节、headers、tools、既有hash输入、消息数、输入成本、缓存TTL及迁移资格均保持原处理方式。已有子代理绑定不会因为识别规则新增支持而重新分类或再次抽取预留机会。

检查父历史时区分内容块、元数据与工具业务参数：支持`tool_use.caller = {"type":"direct"}`，以及仅位于`tool_result.content`内、带字符串`tool_name`的`tool_reference`块；`tool_use.input`中的业务`type`是普通JSON数据。未知caller、未知内容块、媒体及身份冲突仍保守处理。兼容识别只作用于尚未绑定的子代理；已绑定父号的子代理不会突然重新抽签，resume保持原绑定，子代理选号不覆盖父绑定。

父历史任意位置上可识别的纯文本`system`通知仍作为普通历史保留，支持字符串正文和text块数组。原始请求和既有hash输入保持不变；消息数、输入成本和缓存TTL仍按原规则计算。只有首条消息、顶层system和tools生成hash，中间或尾部通知不会单独生成hash。这不放宽独立子代理首轮规则：子代理含任意其他system通知或继承历史，仍不能通过单任务首轮判断；未知结构和媒体仍保守处理。

身份可判定与迁移成本可估算是独立条件。新增工具形状的身份支持**不会扩大**原有保守迁移输入白名单；父历史可用于证明新子代理独立，并不代表父会话本身可以主动迁移。
