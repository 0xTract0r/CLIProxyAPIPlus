...
独立子代理首轮允许一条纯文本user任务，以及可选的一条实测CLI日期辅助消息：role为`system`，content仅含一个text块，正文严格为`Today's date is YYYY-MM-DD.`，可带已支持的ephemeral缓存标记。任意其他system正文、额外历史消息或fork证据都不满足独立首轮条件。日期只在“是否单任务首轮”的身份判断中排除；消息数、输入成本和缓存TTL仍计算它。
